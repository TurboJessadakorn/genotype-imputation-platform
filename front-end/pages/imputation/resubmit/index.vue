            ...
            <div class="input-container basis-1/2">
                ...
                <div class="radio-container">
                    <div class="radio-select">
                        ...
                        <label for="default-radio-1" class="ml-2">GRCh38/hg38</label>
                    </div>
                    <div class="radio-select">
                        ...
                        <label for="default-radio-2" class="ml-2">GRCh37/hg19</label>
                    </div>
                </div>
            </div>
                ...
                <div class="radio-container">
                    <div class="radio-select">
                        <input checked id="default-radio-3" name="inputtype" type="radio" value="pedmap"
                            ...
                            class="radio radio-primary w-4 h-4"
                            required />
                        <label for="default-radio-3" class="ml-2 ">PED MAP format</label>
                    </div>
                    <div class="radio-select">
                        <input id="default-radio-4" name="inputtype" type="radio" value="bedbimfam"
                            ...
                            disabled
                            class="radio radio-primary w-4 h-4" />
                        <label for="default-radio-4" class="ml-2 ">BED BIM FAM format</label>
                    </div>
                    <div class="radio-select">
                        <input id="default-radio-4" name="inputtype" type="radio" value="vcf"
                            v-model="formData.inputtype"
                            disabled
                            class="radio radio-primary w-4 h-4" />
                        <label for="default-radio-4" class="ml-2 ">VCF format</label>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="basis-3/5">
                <div class="radio-select">
                    <input checked id="default-radio-5" type="radio" value="ref_raw1kghg38"
                        ...
                        class="radio radio-primary w-4 h-4"
                        required />
                    <label for="default-radio-5" class="ml-2 ">1,000 Genomes Project (hg38)</label>
                </div>
                <div class="radio-select">
                    <input checked id="default-radio-6" type="radio" value="ref_rawGAsP"
                        v-model="formData.ref_raw" name="Holotype"
                        class="radio radio-primary w-4 h-4" />
                    <label for="default-radio-6" class="ml-2 ">GenomeAsia Pilot (GAsP) Project</label>
                </div>
                <!-- <div class="radio-select">
                    <input checked id="default-radio-7" type="radio" value="ref_rawWGS1175"
                        v-model="formData.ref_raw" name="Holotype"
                        class="radio radio-primary w-4 h-4" />
                    <label for="default-radio-7" class="ml-2 ">WGS1175 reference data</label>
                </div> -->
                <div class="radio-select">
                    <input checked id="default-radio-8" type="radio" value="ref_rawWGSthaiOnly"
                        v-model="formData.ref_raw" name="Holotype"
                        class="radio radio-primary w-4 h-4" />
                    <label for="default-radio-8" class="ml-2 ">1,000 Thais Reference Panel</label>
                </div>
            </div>
        </div>
            ...
            <div class="basis-3/5">
                <div class="radio-select">
                    <input disabled checked id="default-radio-8" type="radio" value="SHAPEIT"
                        ...
                        class="radio radio-primary w-4 h-4"
                        required />
                    <label for="default-radio-8" class="ml-2 ">SHAPEIT5 followed by IMPUTE5</label>
                </div>
            </div>
        </div>
...
export default {
    ...
    mounted() {
        ...
        this.formData.project_id = inputField.split('/')[1];
    },
    methods: {
        setmafValue(value) {
        ...
        uploadFile() {
            ...
            if (this.$route.query.back === "true") {
                this.$axios.put(`/edit_project/${this.$store.state.EditprojectId}/`, this.formData)
                    .then((response) => {
                        this.$store.dispatch('setProjectId', this.formData.project_id);
                        this.editProjectLog();
                        this.$router.push({
                ...
                this.$axios.post("/re_create_project/", this.formData)
                .then((response) => {
                    console.log(response.data)
                    const projectId = response.data.project_id;
                    this.$store.dispatch('setEditProjectId', projectId);
                    this.$store.dispatch('setProjectId', this.formData.project_id);
                    this.createProjectLog();
                    this.$axios.get(`/add-ongoing-project/?user_id=${this.formData.user}&project_id=${projectId}&resubmit=true`)
                    this.$router.push({
                        path: '/imputation/resubmit/upload',
                        query: { projectData: JSON.stringify(this.formData), back: 'true' }
                    });
